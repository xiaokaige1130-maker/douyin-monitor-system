...
class Config:
    ...
    # 监控配置
    check_interval_normal: int = field(default_factory=lambda: int(os.getenv("CHECK_INTERVAL_NORMAL", "10")))  # 非开播期检查间隔（分钟）
    check_interval_live: int = field(default_factory=lambda: int(os.getenv("CHECK_INTERVAL_LIVE", "2")))      # 开播期检查间隔（分钟）
    max_concurrent_checks: int = field(default_factory=lambda: int(os.getenv("MAX_CONCURRENT_CHECKS", "5")))  # 最大并发检查数
